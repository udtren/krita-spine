...
- `exporter.py` contains:
  - `active_group_export_name(document)` — validates the active node is a group
    layer and builds `parentGroupName_activeGroupName` (tags stripped).
  - `ExportSettings` / `ExportResult` dataclasses.
  - `SpineExporter` — walks the layer tree, prepares layers, writes PNGs and JSON.
  - `_effective_visible` — hidden layers and layers inside hidden groups are
    always skipped.
